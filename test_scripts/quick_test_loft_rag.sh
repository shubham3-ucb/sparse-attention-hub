#!/usr/bin/env bash
# Quick test: LOFT RAG HotpotQA 32k with 3 samples
set -euo pipefail
...
echo "=========================================="
echo "QUICK TEST: LOFT RAG HotpotQA 32k (3 samples)"
echo "=========================================="
...
cd "$(dirname "$0")/.."
python3 test_scripts/test_loft_rag.py 2>&1 | tee test_log_loft_llama3.txt

# Extract output directory from Python output
OUTPUT_DIR=$(grep "OUTPUT_DIR_ACTUAL:" test_log_loft_llama3.txt | cut -d' ' -f2)
if [ -n "${OUTPUT_DIR}" ]; then
    mv test_log_loft_llama3.txt "${OUTPUT_DIR}/test_log.txt"
    echo ""
