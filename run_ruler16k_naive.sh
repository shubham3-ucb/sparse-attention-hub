...
# Naive RULER 16k runner: Simple dense vs sparse, no repositioning, no unrope
# Based on: output_llama31_8b_128k_naive_pcs1024_hs050
...
ORACLE_TOPK_HEAVY_SIZE="${ORACLE_TOPK_HEAVY_SIZE:-0.1}"  # 50% sparsity
RULER_TASKS="${RULER_TASKS:-}"  # Empty = all 13 tasks

# Naive settings (no repositioning, no unrope, no debug)
ENABLE_POSITION_REASSIGNMENT=0
COMPARE_MASK_ROPED_VS_UNROPED=0
EXTEND_CONTEXT=0
SPARSE_DEBUG=0
MAX_CONTEXT_LENGTH="${MAX_CONTEXT_LENGTH:-2147483647}"

# Output directory - match earlier naming pattern: output_llama31_8b_128k_naive_pcs1024_hs050
# Format: output_{model_short}_ruler16k_naive_pcs{chunk_size}_hs{heavy_size}
...
# Format heavy_size as 3-digit (e.g., 0.5 -> 050, 0.1 -> 010)
hs_int=$(awk "BEGIN {printf \"%03d\", ${ORACLE_TOPK_HEAVY_SIZE} * 100}")
OUTDIR="${REPO}/output_${model_short}_ruler16k_naive_pcs${PREFILL_CHUNK_SIZE}_hs${hs_int}"
...
# Create output directory (mkdir -p creates parent directories if needed)
mkdir -p "${OUTDIR}" || {
...
# Validation checks
echo "[INFO] Naive RULER 16k run (no repositioning, no unrope)"
echo "[INFO] ================================================="
...
echo "[INFO] Settings: ENABLE_POSITION_REASSIGNMENT=0, EXTEND_CONTEXT=0, SPARSE_DEBUG=0"
echo "[INFO] ================================================="

# Validate NUM_SAMPLES is positive
if ! [[ "${NUM_SAMPLES}" =~ ^[0-9]+$ ]] || [ "${NUM_SAMPLES}" -le 0 ]; then
  echo "[FATAL] NUM_SAMPLES must be a positive integer, got: ${NUM_SAMPLES}"
  exit 1
fi

# Validate ORACLE_TOPK_HEAVY_SIZE is between 0 and 1
...
# Validate PREFILL_CHUNK_SIZE is positive
...
# Verify naive settings are correct
if [ "${ENABLE_POSITION_REASSIGNMENT}" != "0" ]; then
  echo "[WARNING] ENABLE_POSITION_REASSIGNMENT should be 0 for naive run, got: ${ENABLE_POSITION_REASSIGNMENT}"
fi
if [ "${EXTEND_CONTEXT}" != "0" ]; then
  echo "[WARNING] EXTEND_CONTEXT should be 0 for naive run, got: ${EXTEND_CONTEXT}"
fi
if [ "${SPARSE_DEBUG}" != "0" ]; then
  echo "[WARNING] SPARSE_DEBUG should be 0 for naive run, got: ${SPARSE_DEBUG}"
fi
...
echo "[RUN] Starting naive RULER 16k benchmark..."
...
echo "[DONE] Naive RULER 16k benchmark completed → ${OUTDIR}"
