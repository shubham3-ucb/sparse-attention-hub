...
EOF
  ...
  RUN_CMD="OUT=\"${outdir}\" MODEL_NAME=\"${MODEL_NAME}\" PREFILL_CHUNK_SIZE=${PREFILL_CHUNK_SIZE} EXTEND_CONTEXT=1 ENABLE_POSITION_REASSIGNMENT=1 COMPARE_MASK_ROPED_VS_UNROPED=1 SPARSE_DEBUG=1 MAX_CONTEXT_LENGTH=${MAX_CONTEXT_LENGTH} OUTPUT_DIR=\"${outdir}\" SPARSE_LOG_PATH=\"${outdir}/hf_prefill.log\" ORACLE_TOPK_HEAVY_SIZE=${heavy_size} NUM_SAMPLES=${NUM_SAMPLES} python test_sparse_oracle.py > \"${outdir}/log.txt\" 2>&1"
